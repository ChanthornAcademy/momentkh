...
;(function (global, factory) {
	...
	function formatKhmerDate({day, month, moment}, format) {
		if (format === null || format === undefined) {
			...
			return config.postformat(
				`ថ្ងៃ${config.weekdays[dayOfWeek]} ${moonDay.count}${
					config.moonStatus[moonDay.moonStatus]
				} ខែ${config.lunarMonths[month]} ឆ្នាំ${
					config.animalYear[animalYear]
				} ${config.eraYear[eraYear]} ពុទ្ធសករាជ ${beYear}`
			)
			...
			return config.postformat(
				format.replace(
					new RegExp(Object.keys(formatRule).join("|"), "g"),
					function (matched) {
	...
	function findLunarDate(target) {
		...
		if (differentFromEpoch > 0) {
			while (
				Moment.duration(target.diff(epochMoment), "milliseconds").asDays() >
				getNumerOfDayInKhmerYear(
					getMaybeBEYear(epochMoment.clone().add(1, "year"))
				)
			) {
				epochMoment.add(
					getNumerOfDayInKhmerYear(
			...
			do {
				epochMoment.subtract(
					getNumerOfDayInKhmerYear(getMaybeBEYear(epochMoment)),
					"day"
				)
			} while (
				Moment.duration(epochMoment.diff(target), "milliseconds").asDays() > 0
			)
		}
		...
		while (
			Moment.duration(target.diff(epochMoment), "milliseconds").asDays() >
			getNumberOfDayInKhmerMonth(khmerMonth, getMaybeBEYear(epochMoment))
		) {
			epochMoment.add(
				getNumberOfDayInKhmerMonth(khmerMonth, getMaybeBEYear(epochMoment)),
				"day"
		 ...
		 */
		const totalDaysOfTheMonth = getNumberOfDayInKhmerMonth(
			khmerMonth,
			getMaybeBEYear(target)
		)
	...
	return {Moment, getKhNewYearMoment2}
})
